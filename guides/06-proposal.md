...
提案書の作成、回答シートの記入、スピーカーノートの生成を行うフェーズ。提案書は原則として単冊で構成し、RFP指定またはコンテキスト超過時のみ分冊する。
...
```
proposal-strategy.md のセクション3「提案書構成案」を読んで、提案書の構成（章立て・想定枚数）を確認して。
原則は単冊。RFPが分冊を指定している場合、または40スライドを超えてコンテキスト品質維持が困難な場合のみ分冊を検討して。
エグゼクティブ・サマリー以外の重複記述を避け、MECE（漏れなくダブりなく）を徹底すること。
```
...
- **顧客向けスライドに内部用語（`ADR`、`ADR-NNN`、`[AUTO]` マーク、`REF-XXX` / `SVC-XXX` 等の内部ID）を出さない**。`architecture-policy.md` の ADR を提案書へ転記する際は「主要な設計判断」「アーキテクチャ選定の考え方」等の平易な表現に言い換える（詳細は README.md「Internal Terminology」参照）
...
| 意思決定時 | Key Decisions に判断内容と理由を追記（提案書構成、トーン決定等） |
