...
| TCO概算 | `source/rfp_answer_output/` | 初期費用 + ランニング費用の総合試算 |
...
6. バッファ・リスク引当の方針
出力先: source/rfp_answer_output/estimation-policy.md
```
...
出力先: source/rfp_answer_output/wbs.md
```
...
- プラットフォーム利用料（年額）
- 合計TCO（5年間）
出力先: source/rfp_answer_output/cost-breakdown.md
```
