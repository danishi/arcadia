...
|------|--------|-------------|
| `proposal-strategy.md` | `source/rfp_answer_output/` | Phase 2で確定した提案戦略 |
...
| プラットフォーム機能マッピング | `source/rfp_answer_output/` | Phase 2で生成した適合度表 |
...
|-------------|----------|-------------|
| `architecture-policy.md` | `source/rfp_answer_output/architecture-plan/` | 設計原則、ADR、セキュリティ方針、インフラ方針 |
| 論理構成図 (`.drawio`) | `source/rfp_answer_output/architecture-plan/` | フェーズ別の論理アーキテクチャ図 |
| 物理構成図 (`.drawio`) | `source/rfp_answer_output/architecture-plan/` | フェーズ別の物理/インフラ構成図 |
| `ph1-migration-requirements.md` | `source/rfp_answer_output/migration-plan/` | 移行要件定義書 |
| 移行設計図 (`.drawio`) | `source/rfp_answer_output/migration-plan/` | 現行構成、移行構成、データフロー、スケジュール |
...
出力先: source/rfp_answer_output/architecture-plan/architecture-policy.md
```
...
出力先: source/rfp_answer_output/architecture-plan/logical-architecture-ph1.drawio
```
...
出力先: source/rfp_answer_output/architecture-plan/physical-architecture-ph1.drawio
```
...
5. 切替判定基準
6. リスクと対策
出力先: source/rfp_answer_output/migration-plan/ph1-migration-requirements.md
```
...
出力先: source/rfp_answer_output/migration-plan/
```
