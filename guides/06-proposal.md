...
|------|--------|-------------|
| `proposal-strategy.md` | `source/rfp_answer_output/` | Winテーマ、差別化ポイント、スコープ |
| `proposal-items-checklist.md` | `source/rfp_answer_output/` | 提出物一覧とステータス |
| `architecture-policy.md` | `source/rfp_answer_output/architecture-plan/` | 設計方針、ADR |
| 構成図 (`.drawio`) | `source/rfp_answer_output/architecture-plan/` | 論理/物理構成図 |
| 移行設計図 (`.drawio`) | `source/rfp_answer_output/migration-plan/` | 移行関連図面 |
| `estimation-policy.md` | `source/rfp_answer_output/` | 見積方針、コスト構造 |
...
|-------------|----------|-------------|
| 提案書ボリューム (PPTX) | `output/Vol{N}-{topic}.pptx` | PPTX method: PowerPointファイル |
| 提案書ボリューム (画像) | `output/Vol{N}-{topic}/slide-{NN}.png` | NanoBanana method: スライド画像 |
| 回答シート (XLSX) | `output/` | RFP所定の回答フォーマット |
| スライド設計書 (MD) | `source/rfp_answer_output/` | 各ボリュームのMarkdown版設計書 |
...
- 使用する図面（あれば）
- スピーカーノート
出力先: source/rfp_answer_output/vol{N}-{topic}-slides.md
```
...
```
vol{N}-{topic}-slides.md にスピーカーノートを追加して。
...
#### PPTX method（`SLIDE_METHOD=pptx` の場合）

```
vol{N}-{topic}-slides.md を元にPPTXファイルを生成して。
出力先: output/Vol{N}-{topic}.pptx
```

#### NanoBanana method（`SLIDE_METHOD=nanobanana` の場合）

```
vol{N}-{topic}-slides.md を元に、NanoBananaスキルでスライド画像を1枚ずつ生成して。
出力先: output/Vol{N}-{topic}/slide-01.png 〜 slide-XX.png
アスペクト比: 16:9
```
...
| `nanobanana` | AI画像生成によるスライド作成（NanoBanana method 選択時） |
| Plugin: `document-skills` | PPTX / XLSX ファイル生成（PPTX method 選択時） |
