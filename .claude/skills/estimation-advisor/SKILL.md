...
- **工数の単位は必ず「人月」と日本語で表記する**。`PM` / `P/M` / `MM` / `M/M` / `man-month` 等の略語は工数単位として**使用禁止**（英語圏の Person Month と混同するため）。
- ロール名としての `PM` は使用せず、プロジェクトマネージャーを指す場合は `PjM` と表記する（`PM/PMO費` → `PjM/PMO費`、`PM/PL` → `PjM/PL` 等）。
