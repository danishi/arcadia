...
__PROJECT_SLUG__/                        # "Use this template" で生成したリポジトリ
  README.md                              # 本ファイル
  DESIGN.md                              # デザインシステム単一真実ソース（Phase 2.5 で確定）
  ...
  guides/
    ...
    04-design.md                         # (Phase 2.5 + 3) デザインシステム・アーキテクチャ・移行計画
  ...
  .claude/
    ...
    agents/
      researcher.md                      # RFP調査・品質チェック（Pre-Phase, Phase 1, 7）
      strategist.md                      # 提案戦略策定（Phase 2）
      architect.md                       # アーキテクチャ設計（Phase 3）
      estimator.md                       # 見積策定（Phase 4）
      writer.md                          # 提案書作成（Phase 5）
      developer.md                       # デモアプリ開発（Phase 6）
      setup-generator.md                 # セットアップファイル生成（/setup 用）
    skills/
  ...
  templates/
    ...
    docs/                                # 提案ドキュメントテンプレート
      ...
      DESIGN.md.tmpl                     # デザインシステム骨格（ARCADIA ニュートラルデフォルト）
      architecture-plan/
  ...
  demo-app/                              # Next.js デモアプリ（最小スキャフォールド。画面は demo-builder が生成）
...
| 2.5 | **Design System** | 色・タイポ・コンポーネント規約の確定 | DESIGN.md | 20/80 |
...
**Phase 2.5 は Phase 5（提案書）と Phase 6（デモ）の前提条件**。`writer` / `developer` エージェントは `DESIGN.md` を必ず参照し、色・タイポ・コンポーネントを統一する。
...
2. **デモアプリの方向性ヒアリング**（画面種類・ドメイン・テーマカラー）
3. **実行モードの選択**（スタンダード or フルオート）
4. `.claude/CLAUDE.md` にプロジェクト固有の値とデモ仕様を反映
5. RFP ドキュメント用のディレクトリ構造を作成（`source/`、`output/` 等）
6. `.mcp.json`、`.gitignore`、`.env.example` を生成
7. `output/plan/` にドキュメントテンプレートを配置
8. フルオートモード選択時: `/auto-run` で Phase 1〜7 を自動実行
...
| `demo-app/` | デモアプリ（最小 Next.js スキャフォールド。`/setup` のヒアリング結果に基づき `demo-builder` が画面を生成） | Phase 6 (Demo) |
...
`.claude/agents/` に定義されたロール特化サブエージェントは、コマンド実行時に Agent ツールから呼び出される。各フェーズの実行を委譲し、親コンテキストの消費を最小化する。

| エージェント | 担当 Phase | 主な責務 | 呼び出し元 |
|-------------|-----------|---------|-----------|
| `researcher` | Pre, 1, 7 | RFP分析、会社情報取得、品質レビュー | `/auto-run`, `/setup` |
| `strategist` | 2, 2.5 | Win戦略定義、スコープ確認、DESIGN.md 確定 | `/auto-run` |
| `architect` | 3 | アーキテクチャ設計、論理構成図 | `/auto-run` |
| `estimator` | 4 | 見積方針書、WBS、コスト算出 | `/auto-run` |
| `writer` | 5 | スライド設計書、PPTX/画像生成（DESIGN.md 強制参照） | `/auto-run` |
| `developer` | 6 | デモアプリ画面実装、ビルド検証（DESIGN.md 強制参照） | `/auto-run` |
| `setup-generator` | — | 資料整理、テンプレート展開 | `/setup` |
...
### Subagent-Delegated Execution

`/auto-run`（フルオート）および `/setup` の重いステップでは、各フェーズの実行を Claude Code の Agent ツールによるサブエージェントに委譲する。親エージェントは「オーケストレーター」として `phase-state.md` の管理とフェーズ間の受け渡しに専念し、実際のファイル操作・成果物生成はサブエージェント内で完結する。

```
[親エージェント: オーケストレーター]
    │
    ├── phase-state.md 読取 → サブエージェントプロンプト構築
    ├── Agent ツール起動 → [サブエージェント: Phase N 実行]
    │                         ├── ガイド・スキル読み込み
    │                         ├── 成果物生成
    │                         └── 構造化サマリーを返却
    ├── 結果に基づき phase-state.md 更新
    └── 次のフェーズへ
```

**設計原則:**
- 親コンテキストの消費を最小化し、7 フェーズを 1 セッションで完走できる確率を最大化
- スキルは自動継承されないため、サブエージェントプロンプトに明示的にスキルパスを含める
- サブエージェントはネスト不可（他のサブエージェントを起動できない）ため、各フェーズは自己完結する
- 詳細は `.claude/commands/auto-run.md` および `.claude/commands/setup.md` の「サブエージェント実行戦略」セクション参照
...
1. `/setup` Phase D でデモの方向性（画面種類・ドメイン・テーマ）をヒアリング
2. `demo-builder` スキルがヒアリング結果からゼロベースで画面・コンポーネント・モックデータを生成
3. プラットフォーム API を接続
4. API 未接続時はモックにフォールバック

### Unified Design System via DESIGN.md

Google Stitch の [DESIGN.md](https://stitch.withgoogle.com/docs/design-md/overview) 仕様に準拠した単一真実ソースで、スライド（Phase 5）とデモアプリ（Phase 6）のトーン・色・タイポ・コンポーネントを統一する。

```
/setup              → templates/docs/DESIGN.md.tmpl を DESIGN.md に展開
                       （ARCADIA ニュートラルデフォルトの骨格）
  ↓
Phase 2.5           → strategist が Win 戦略・クライアント像に基づき
                       DESIGN.md のプリセットとトーンを確定
  ↓
Phase 5 (writer)    → スライド設計書・per-volume design.md が DESIGN.md を継承
Phase 6 (developer) → Tailwind 変数が DESIGN.md §8 から直接適用
```

**優先順位**: (1) クライアントブランド色指定 → `client-brand` / (2) 業界慣例による調整 → `custom` / (3) 情報なし → ARCADIA ニュートラル（デフォルト）を維持。

**強制参照**: writer / developer は `DESIGN.md` 未確定の場合 `blocked` ステータスを返し、Phase 2.5 の実行を促す。素の Tailwind カラートークン（`bg-slate-*` 等）の使用は禁止。

---

## Internal Terminology（内部用語集）

ARCADIA の内部成果物・サブエージェント間通信で使用する用語。**これらは内部設計の便宜上の用語であり、顧客向けアウトプット（提案書スライド、PPTX、PDF、回答シート、デモアプリ UI 等）には表出させない**。顧客向けには日本の実務に馴染む平易な表現へ言い換える。

### ADR（Architecture Decision Record）

| 項目 | 内容 |
|------|------|
| 意味 | アーキテクチャ決定記録。技術・ベンダー選定等の**判断1件ごとの追跡単位**（意思決定の番号） |
| 番号体系 | `ADR-NNN`（例: `ADR-001`〜`ADR-018`） |
| 格納先 | `output/plan/architecture-plan/architecture-policy.md` の「1.2 アーキテクチャ決定記録（ADR）」表＋詳細セクション |
| 生成者 | `architect` エージェント（Phase 3） |
| 役割 | (1) フェーズ間ハンドオフ契約（Phase 3 → 4/5/6/7 の参照キー）、(2) RFP要件↔質問票回答↔設計判断の3層トレーサビリティ基準点、(3) `phase-state.md` Key Decisions の状態管理記録単位 |
| 内部通信フォーマット | サブエージェント戻り値で `- [AUTO] ADR-NNN: 決定内容（根拠: ...）` として使用（`.claude/agents/architect.md` 参照） |
| **顧客向け言い換え** | 提案書・スライドでは **「アーキテクチャ選定の考え方」「主要な設計判断」「技術選定の根拠」** 等の平易な表現に置換。`ADR-NNN` の番号も出さない |

> **なぜADRを振るか**: 番号がないと「なぜそう決めた？」をレビュー毎に掘り返し、判断変更履歴（Supersedes）が追えず、Phase 7 の RFP 準拠監査でトレーサビリティが切れる。フェーズ並行実行（Phase 3/4/6 同時進行）はADR番号参照を前提に成立する。

### その他の番号体系

| 種別 | プレフィックス | 用途 | 顧客向け表出 |
|------|--------------|------|:---:|
| 判断番号 | `ADR-NNN` | アーキテクチャ意思決定の追跡 | 不可 |
| 要件番号 | `REQ-xxx` / `BRQ` / `CRQ` / `OP` | RFP要件との対応（トレーサビリティマトリクス） | RFP準拠の文脈で可 |
| 実績ID | `REF-xxx` | `org-data/company-profile.md` の導入実績参照 | 必要時に社名・案件名で表現 |
| サービスID | `SVC-xxx` | `org-data/service-catalog.md` のサービス参照 | サービス正式名称で表現 |
| タスクID | `TASK-N` | `tasks.md` の内部タスク管理 | 不可 |
...
| `proposal-all.pdf` | `proposal-all.pdf` | 全ボリューム結合 PDF（`output/` 配下、`document-skills` プラグインが生成） |
