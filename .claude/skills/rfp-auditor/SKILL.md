---
name: rfp-auditor
description: "RFP要件に対する成果物のマッチ・アンマッチ・ヌケモレをチェックするスキル。プロジェクトのsource/配下の原典資料を参照し、エビデンスに基づく妥当性検証を行う。"
user-invocable: true
...
本スキルは `source/` 配下のRFP資料・現行システム設計書を**エビデンスソース**として活用し、
...
このファイルには `source/` 配下の全資料が分類されており、各要件IDに対応する**エビデンスソース**が逆引きマップとして整理されている。
...
|------------|-------------------|-------------------|
| デモアプリ（demo-app/） | F, A（該当機能） | 要件定義フォルダ, インターフェース定義フォルダ |
...
- **参照した原典資料**: [実際に読み込んだsource/配下ファイル一覧]
- **チェック日時**: YYYY-MM-DD
- **チェック深度**: 概要/詳細
